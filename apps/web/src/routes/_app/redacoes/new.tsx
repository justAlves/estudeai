...
import { useCreateRedacao } from '@/api/hooks/useRedacoes'
...
export const Route = createFileRoute('/_app/redacoes/new')({
  component: NewRedacaoComponent,
})
...
function NewRedacaoComponent() {
  ...
  const createMutation = useCreateRedacao((reason) => {
  ...
  const handleSave = (sendForCorrection: boolean = false) => {
    if (!title.trim() || !theme.trim() || !content.trim() || !bank.trim()) {
      toast.error('Preencha todos os campos antes de salvar')
      return
    }

    // Se vai enviar para correção, verificar limite antes
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto w-full">
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            <Button
              ...
              onClick={() => {
                ...
                // Salvar sem enviar para correção
                handleSave(false)
              }}
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">Escreva sua redação e envie para correção</p>
        </div>
      </div>
      ...
      <div className="flex gap-4 justify-end">
        ...
        <Button
          ...
          disabled={createMutation.isPending || !title.trim() || !theme.trim() || !content.trim() || !bank.trim()}
        >
